...
class _CompanyScreenState extends State<CompanyScreen> {
  ...
  void initState() {
    super.initState();
    sl<AuthRepository>().company().then((company) {
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _save() async {
    ...
    try {
      final company = await sl<AuthRepository>().updateCompany({
...
class _PacksScreenState extends State<PacksScreen> {
  ...
  Future<void> _save() async {
    setState(() => _saving = true);
    try {
      final company = await sl<AuthRepository>().updateCompany({'pack': _selected});
      if (!mounted) return;
      context.read<AuthBloc>().add(AuthCompanyUpdated(company));
